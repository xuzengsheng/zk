...
public class MyBatisPlusGenerator {
    ...
    //数据库名
    private static final String database = "nccs_user";
    //url
    ...
    private static String author = "Monroe";  //作者
    private static String packageName = "com.nccs"; //自定义包名
    private static String[] tablePrefix = null; //自定义表名前缀 数组


    public static void main(String[] args) {
        generate(author, packageName, tablePrefix);
    }
...
//        gc.setBaseColumnList(false);// XML columList
        ...
        pc.setParent(packageName);//总包名
        pc.setEntity("entity.auto");
        pc.setMapper("mapper.auto");
        pc.setService("service.auto");
        pc.setServiceImpl("service.impl.auto");
        pc.setController("controller");
        pc.setXml("mapper.xml.auto");
    ...
    private static TemplateConfig initTemplateConfig(String mapperFile) {
        ...
        tc.setController(null);//设置为null表示重新生成时不会覆盖原始文件

//        File file = new File(mapperFile);
//        if (file.exists()) {
//        //判断当前文件是否已经存在 如果已经存在 则不再重新生成
